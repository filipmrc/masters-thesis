...
\draw[black,thick] ($(ROS CONTROL.north west)+(-1.6,0.6)$)  rectangle ($(UR10 API.south east)+(5,-1)$);
\end{scope}
\draw[black,thick] ($(RIDGEBACK API.north west)+(-0.3,0.4)$)  rectangle ($(UR10 API.south east)+(0.3,-0.4)$);
\draw[black,thick] ($(TASK.north west)+(-6,0.4)$)  rectangle ($(THING KINEMATICS.south east)+(1,-1.3)$);
